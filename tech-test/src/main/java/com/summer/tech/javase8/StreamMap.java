...
public class StreamMap {
	...
	public static void main(String[] args) {
		 ...
		 *
		 * List<Integer> result = list.stream().map(i -> i *
		 * 2).collect(toList());
		 *
		 * System.out.println(result);
		 *
		 * listDish().stream().map(d ->
		 * d.getName()).forEach(System.out::println);
		 *
		 * List<String> dishes = listDish().stream().map(d ->
		 * d.getName()).collect(toList()); System.out.println(dishes);
		 *
		 * //flatmap flat (扁平化)
		 */
