package com.summer.tech.memcached.xmemcache;
...
import com.summer.tech.utils.Md5Util;
...
public class XMemcacheTest {
	...
	public void test1() throws IOException {
		...
		/**
		 * 设置连接池大小，即客户端个数 In a high concurrent enviroment,you may want to pool memcached
		...
		try {
			/**
			 * 第一个是存储的key名称， 第二个是expire时间（单位秒），超过这个时间,memcached将这个数据替换出去，0表示永久存储（默认是一个月)
			 * 第三个参数就是实际存储的数据
			 */
