...
import com.summer.tech.websocket.drawboard.wsmessages.AbstractWebsocketMessage;
import com.summer.tech.websocket.drawboard.wsmessages.BinaryWebsocketMessage;
import com.summer.tech.websocket.drawboard.wsmessages.CloseWebsocketMessage;
...
public class Client {
	...
	/**
	 * Contains the messages which are buffered until the previous send
	...
	/**
	 * If this client is closing. If <code>true</code>, new messages to send
	 * will be ignored.
	 */
	private volatile boolean isClosing = false;
	/**
	 * The length of all current buffered messages, to avoid iterating over a
	 * linked list.
	 */
	 ...
	 * remaining messages have been sent to the Client and then close the
	 * Websocket session.
	 */
	public void close() {
	...
	/**
	 * Sends the given message asynchronously to the client. If there is already
	 * a async sending in progress, then the message will be buffered and sent
	 * when possible.<br>
	 * <br>
	 *
				...
				if (isSendingMessage) {
					...
					if (messagesToSend.size() >= 1000 || messagesToSendLength >= 1000000) {
						...
						try {
							...
							// is still in the process of sending data, so there
							// probably should
							// be another way to abort the Websocket connection.
							// Ideally, there should be some abort() method that
							// cancels the
	...
	private final SendHandler sendHandler = new SendHandler() {
		...
		public void onResult(SendResult result) {
			if (!result.isOK()) {
				...
				// other messages.
				// As a precaution, we close the session (e.g. if a send timeout
				// occurred).
